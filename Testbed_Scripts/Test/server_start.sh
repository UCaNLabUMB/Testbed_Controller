...
#
# Input: -<flag letter #1> ip suffix -<n> number of ports to be open or -<flag letter #1> ip suffix -<p> specific ports numbers. **For Help, enter -h**
...
server() # takes array from base_pprt to base_port + 1. e.g if N=3, then ports 5201, 5202 and 5303 will be started.
{
	counter=1
	N=$OPTARG
	base_port=5200
...
while getopts 'ha:n:p' OPTION; do
	case "$OPTION" in
		h)
			help;;
		a)
...
while [ $i -lt ${#my_ports[@]} ]; do # loop for number of ports
	ssh victoriap@10.1.1.$ip iperf3 -s -p ${my_ports[$i]} &
	i=$((i + 1))
done
